...
def clean_data(df):
    ...
    try:
        ...
        logging.warning(f"Erro ao converter Fecha para DATE: {e}")
        logging.warning(f"Valores brutos de Fecha: {df_clean['Fecha'].head(10).to_string()}")
...
def create_sqlite_db(df, db_dir):
    ...
    logging.info(f"Criando banco SQLite e tabela 'medicoes': {db_file}")
    ...
    with sqlite3.connect(db_file) as conn:
        ...
        conn.commit()
    logging.info(f"Tabela 'medicoes' criada e dados inseridos com sucesso em: {db_file}")
